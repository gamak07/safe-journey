...
export async function POST(req: Request) {
  try {
    ...
    if (!backendRes.ok) {
      const msg = result?.message || result?.error || result?.text || "Login failed";
    ...
    const token = result?.token || result?.jwt || result?.data?.token || result?.accessToken;

    const response = NextResponse.json({ success: true, user: result }, { status: 200 });
